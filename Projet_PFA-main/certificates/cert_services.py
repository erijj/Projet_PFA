...
def _draw_header(c, W, H):
    """En-tête : logo image + titre principal."""
    # ── Logo SmartCert (image logo.png) ─────────────
    logo_x, logo_y = W/2, H - 38*mm
    BASE = os.path.dirname(os.path.abspath(__file__))
    logo_path = os.path.join(BASE, '..', 'shared', 'logo.png')

    if os.path.exists(logo_path):
        # Afficher le vrai logo image
        logo_w, logo_h = 28*mm, 28*mm
        try:
            img = ImageReader(logo_path)
            c.drawImage(img, logo_x - logo_w/2, logo_y - logo_h/2,
                        width=logo_w, height=logo_h,
                        mask='auto', preserveAspectRatio=True)
        except Exception:
            # Fallback cercle si image corrompue
        ...
        # Fallback cercle si logo.png introuvable
        c.setFillColor(C_TEAL)
        c.circle(logo_x, logo_y, 14*mm, fill=1, stroke=0)
        c.setFillColor(C_WHITE)
        c.setFont('Helvetica-Bold', 18)
